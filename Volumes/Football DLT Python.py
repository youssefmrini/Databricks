...
def games2000_bronze():
  return (
  spark.readStream.format("cloudFiles").schema("Round int, Date String,`Team 1` String,FT String , `Team 2` string").option("cloudFiles.format", "csv").option("header","True").load("/Volumes/football_uk/allseasons/dataset/2000s")
  )
...
def games2010_bronze():
  return (
  spark.readStream.format("cloudFiles").schema("Round int, Date String, `Team 1` String, FT String ,`Team 2` string").option("cloudFiles.format", "csv").option("header","True").load("/Volumes/football_uk/allseasons/dataset/2010s")
  )
