...
# MAGIC %sql 
# MAGIC CREATE DATABASE IF NOT EXISTS feature_store_taxi_MIQ;
...
fs.create_table(
    name="feature_store_taxi_MIQ.trip_pickup_features",
    primary_keys=["zip", "ts"],
    df=pickup_features,
...
fs.create_table(
    name="feature_store_taxi_MIQ.trip_dropoff_features",
    primary_keys=["zip", "ts"],
    df=dropoff_features,
    partition_columns="yyyy_mm",
...
metadata=fs.get_table("feature_store_taxi_miq.trip_dropoff_features")
print(metadata.primary_keys)
print(metadata.description)
print(metadata.partition_columns)

# COMMAND ----------

table=fs.read_table("feature_store_taxi_MIQ.trip_dropoff_features")
display(table)
...
# MAGIC FROM   feature_store_taxi_kerry.trip_dropoff_features
...
pickup_features_table = "feature_store_taxi_MIQ.trip_pickup_features"
dropoff_features_table = "feature_store_taxi_MIQ.trip_dropoff_features"
...
dropoff_feature_lookups = [
    ...
    FeatureLookup( 
      table_name = dropoff_features_table,
      feature_names = "dropoff_is_weekend",
      lookup_key = ["dropoff_zip", "rounded_dropoff_datetime"],
    ),
]
...
fs.log_model(
  ...
  registered_model_name="taxi_example_fare_MIQ"
)
...
# Get the model URI
latest_model_version = get_latest_model_version("taxi_example_fare_MIQ")
model_uri = f"models:/taxi_example_fare_MIQ/{latest_model_version}"
