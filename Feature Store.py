...
import mlflow
from pyspark.sql.functions import struct, lit, to_timestamp,current_timestamp

output_df = with_predictions_reordered.withColumn("model_version", lit(latest_model_version)).withColumn("inference_timestamp", current_timestamp())
        #.withColumn("fare_amount", table.select("fare_amount"))
display(output_df)
    
output_df.write.format("delta").mode("append").option("mergeSchema",True).option("delta.enableChangeDataFeed", "true").saveAsTable("ml.taxi_example.predict")
...
# Delete the monitor for table_name=ml.taxi_example.predict
lm.delete_monitor(table_name="ml.taxi_example.predict")
...
lm.create_monitor(
    table_name="ml.taxi_example.predict",
    profile_type=lm.InferenceLog(
        problem_type="regression",
        prediction_col="predicted_fare_amount",
        timestamp_col="inference_timestamp",
        granularities=["1 day"],
        model_id_col="model_version",
        label_col="fare_amount" # optional
    ),
    slicing_exprs=["dropoff_is_weekend=1","dropoff_is_weekend=0"],
    output_schema_name="ml.taxi_example"
)
